...
from multimodn.encoders import MLPFeatureEncoder
...
def main():
    ...
    features = ['Fare', 'Pclass', 'Age', 'Relatives', 'Embarked', 'Cabin_num']
    targets = ['Survived']
    ...
    learning_rate = 0.01
    epochs = 40 if not args.epoch else args.epoch
    ...
    dataset = TitanicDataset(
        features,
        targets,
        dropna=not keep_missing_values,
        std=True,
    ).featurewise_dataset()
    ...
    ##############################################################################
    encoders = [MLPFeatureEncoder(state_size, 5, F.relu) for _ in features]
